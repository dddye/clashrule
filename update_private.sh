...
if [ $1 == 'host' ]; then
        ...
        if [[ -f /etc/xiaoya/proxy.txt ]] && [[ -s /etc/xiaoya/proxy.txt ]]; then
                ...
                docker run -d --env HTTP_PROXY="$proxy_url" --env HTTPS_PROXY="$proxy_url" --env no_proxy="*.aliyundrive.com" --network=host -v /etc/xiaoya:/data --restart=always --name=xiaoya-hostmode dockerproxy.com/xiaoyaliu/alist:hostmode
        else
                docker run -d --network=host -v /etc/xiaoya:/data --restart=always --name=xiaoya-hostmode dockerproxy.com/xiaoyaliu/alist:hostmode
        fi
        exit
fi
fi
...
if [ ! -s /etc/xiaoya/docker_address.txt ]; then
        echo "http://$localip:1234" > /etc/xiaoya/docker_address.txt
fi
docker stop xiaoya
docker rm xiaoya
...
if [[ -f /etc/xiaoya/proxy.txt ]] && [[ -s /etc/xiaoya/proxy.txt ]]; then
        proxy_url=$(head -n1 /etc/xiaoya/proxy.txt)
        docker run -d --env HTTP_PROXY="$proxy_url" --env HTTPS_PROXY="$proxy_url" --env no_proxy="*.aliyundrive.com" -v /etc/xiaoya:/data --restart=always --name=xiaoya dockerproxy.com/xiaoyaliu/alist:latest
else
        docker run -d -p 1234:80 -v /etc/xiaoya:/data --restart=always --name=xiaoya dockerproxy.com/xiaoyaliu/alist:latest
fi
